                ...
                'metrics'=>
                      ['companies'=>DB::table('jobs')->whereNotNull('employer_id')->distinct('company_id')->count('company_id'),
                        ...
                        'students'=>User::where(fn($q)=>$q->where('role_code',1)->orWhere('role','student'))->count(),'jobs'=>DB::table('jobs')
                            ...
                            ->limit(20)
                            ->get(),
                            'deleted'=>DB::table('deleted_job_events')
                            ->leftJoin('users','users.id','=','deleted_job_events.employer_id')
                            ->leftJoin('companies','companies.id','=','deleted_job_events.company_id')
                            ->select('deleted_job_events.*','users.email','companies.name as company_name')
                            ->latest('deleted_job_events.deleted_at')
                            ->limit(20)
                            ->get(),
