...
class AdminJobApprovalController extends Controller
{
    ...
    {
        abort_unless($job->status === 'pending', 422, 'Only pending jobs can be published.');
        $job->update(['status' => 'published', 'is_active' => true, 'published_at' => now(), 'posted_at' => $job->posted_at ?? now()]);
